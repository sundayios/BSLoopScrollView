...
- (void)commonInit
{
    _scrollDirection = BSLoopScrollViewScrollDirectionHorizontal;
...
- (void)layoutSubviews
{
    ...
    if (_scrollDirection == BSLoopScrollViewScrollDirectionHorizontal) {
        ...
        for (int i = 0; i < _pageCount; i++) {
            UIView *contentView = self.contentViews[i];
            contentView.frame = CGRectMake(0, (i + 1) * height, width, height);
        }
        self.scrollView.contentSize = CGSizeMake(width, (_pageCount + 2) * height);
        [self.scrollView scrollRectToVisible:CGRectMake(0, height, width, height) animated:NO];
    }
}
...
- (UIPageControl *)pageControl
{
    if (!_pageControl) {
        ...
        [self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"V:[pageControl]-margin-|" options:0 metrics:@{@"margin":@(-6)} views:NSDictionaryOfVariableBindings(pageControl)]];
    }
    return _pageControl;
}
...
- (void)handleAutoScroll:(NSTimer *)timer
{
    ...
    if (_scrollDirection == BSLoopScrollViewScrollDirectionHorizontal) {
         contentOffset = CGPointMake(_currentPage * width, 0);
        ...
        contentOffset = CGPointMake(0, _currentPage * height);
        if (contentOffset.y < height) {
            self.lastView.frame = CGRectMake(0, 0, width, height);
        }else if (contentOffset.y > _pageCount * height) {
            self.firstView.frame = CGRectMake(0, (_pageCount + 1) * height, width, height);
        }
    }
...
- (void)handleTapAction:(UITapGestureRecognizer *)tap
{
    NSInteger index = 0;
    if (_scrollDirection == BSLoopScrollViewScrollDirectionHorizontal) {
        index = [tap locationInView:tap.view].x / tap.view.bounds.size.width - 1;
    }else {
        index = [tap locationInView:tap.view].y / tap.view.bounds.size.height - 1;
    }
...
- (void)resetPosition
{
    ...
    if (_scrollDirection == BSLoopScrollViewScrollDirectionHorizontal) {
        ...
        _currentPage = self.scrollView.contentOffset.x / width - 1;
    }else {
        self.firstView.frame = CGRectMake(0, height, width, height);
        self.lastView.frame = CGRectMake(0, _pageCount * height, width, height);
        if (self.scrollView.contentOffset.y == (_pageCount + 1) * height) {
            [self.scrollView scrollRectToVisible:self.firstView.frame animated:NO];
        }else if (self.scrollView.contentOffset.y == 0) {
            [self.scrollView scrollRectToVisible:self.lastView.frame animated:NO];
        }
        _currentPage = self.scrollView.contentOffset.y / height - 1;
    }
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    ...
    if (_scrollDirection == BSLoopScrollViewScrollDirectionHorizontal) {
        ...
        if (_pageCount == 2) {
            ...
            if (scrollView.contentOffset.y > _pageCount * height) {
                self.firstView.frame = CGRectMake(0, (_pageCount + 1) * height, width, height);
            }else {
                self.firstView.frame = CGRectMake(0, height, width, height);
            }
            ...
            if (scrollView.contentOffset.y < height) {
                self.lastView.frame = CGRectMake(0, 0, width, height);
            }else if (scrollView.contentOffset.y > _pageCount * height) {
                self.firstView.frame = CGRectMake(0, (_pageCount + 1) * height, width, height);
            }
        }
    }
